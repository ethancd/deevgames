...
report={'method':'For each speech clip, locate its highest-energy excerpt in the decoded final video audio using normalized cross-correlation. This checks both presence and timing after rendering and mastering.','clips':out,'minimumCorrelation':min(x['correlation'] for x in out),'maxAbsoluteOffsetSeconds':max(abs(x['offsetSeconds']) for x in out)}
