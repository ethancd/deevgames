...
it('Medium resets an imminent draw with a mine while ahead',async()=>{
 let s=createInitialGameState();s.inactivityPlies=19;s.players.white.resources=12;s.players.white.resourcesGained=12;
...
it('harness records a real inactivity draw separately from its safety cap',async()=>{
 ...
 expect(record.winType).toBe('inactivity');expect(record.inactivityDraw).toBe(true);expect(record.maxInactivityPlies).toBe(20);expect(record.turns).toBe(11);expect(record.invariantViolation).toBeNull();expect(record.anomalies).toEqual([]);expect(record.players.white.upkeepPaid).toBe(0);
});
