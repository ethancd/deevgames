...
async function setup() {
  ...
  const white = store.create({ name: 'White', matchPolicy: policy, timeControl: 'classical' });
  ...
  const base = { mode: 'pinned', serverUrl, roomId: white.room.id, seed: 42, stateFile: '/private/not-written-binding-test.json',
    expectedMatchPolicy: policy, expectedTimeControl: { delaySeconds: 60, bankSeconds: 1800 }, expectedHandicap: 0 };
...
it.each([false, true])('rejects a hand-edited seat journal before search or pending recovery (pending=%s)', async pending => {
  ...
  const journal: SeatJournal = { version: 2, admission: 'issued', seed: 42, contract: contractFor(config),
    connection: { ...black.credentials, serverUrl: base.serverUrl, player: 'white' },
    ...(pending ? { pending: { expectedRevision: 1, requestId: 'binding-pending-request', actions: [{ type: 'END_ACTION_PHASE' as const }] } } : {}) };
