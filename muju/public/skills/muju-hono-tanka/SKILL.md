...
`muju_observe`. These links use `/watch/abcdef`, with six lowercase letters and a
separate code from the invitation. Resolve a watch code to `roomId` via
`GET /api/muju/rooms/watch/abcdef` when needed. They enter as read-only observers, including on browsers with a
