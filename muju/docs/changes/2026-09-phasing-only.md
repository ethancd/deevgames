...
T2b addendum (2026-09-19 UTC, prepared/uncommitted): T1+T2 are now combined locally at `a092e2fc`. The fixed-work Gate 1 runner, dated amendment proposal and historical-reference audit are delivered under `lab/ai/` and `lab/docs/`. `lab/ai/results/t2b-gate1-pilot-2026-09-19/README.md` records the 16-game pilot: 2,307 legal actions, zero illegal/invariant/anomaly/cap/adapter failures, 211 hard-seat commitments, two inactivity draws. All six L2 behavioral cells met the unchanged T1 bands; hard lost 3/4 Rush games, so no baseline strength claim follows. Four TypeScript configurations and 78 focused tests passed; all replay actions, 128 source hashes and 227 engine-seat work allowances independently verified. Full 1,024-game proposal remains unrun; Gate 1 acceptance is blocked on amendment adoption and missing matching Standard Expand/Balanced margins (Rush historical anchor is qualified by 18 illegal emissions). Frozen preregistration and all production AI/worker/Hard/canonical sources remain unchanged. See the pilot record for every affected DAG node disposition; no deployment or release claimed.

T2c / A1 addendum (2026-09-19 UTC, prepared/uncommitted, base `c43c7280`): adopted A1 pass logic now replaces the historical-reference block; Rush strength is report-only, other strength cells and unchanged behavioral bands gate, and every hard game longer than 10 completed player turns must include a purchase. The saved zero-buy Rush replay exposed all-risk purchase filtering with funded banks; legal risky buys now remain searchable when no safe buy exists. No weight/budget/rule/worker/Hard changes. All 326 scoped AI/lab/server tests, four type configurations and the browser build pass. The new 16-game pilot has 2,686 independently verified actions, zero correctness failures, purchases in every hard seat (five in the formerly zero-buy game), and no must-buy violations. Gate 1 is still unpassed; full rows await a quiet window, all one-pair intervals contain zero, and Expand h0 has 100% inactivity draws outside its unchanged band. Full diagnosis, source identity, per-cell results and every affected DAG disposition: `docs/changes/2026-09-19-gate1-a1-t2c.md` and `lab/ai/results/t2c-gate1-pilot-2026-09-19/README.md`. No outcome-driven tuning, extra pilot rerun, other-worktree write or deployment.
