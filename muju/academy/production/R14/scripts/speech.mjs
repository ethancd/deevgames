...
const pronounce=t=>t.replace(/\bHi\b/g,'Hee').replace(/\bSjor\b/g,'Shore').replace(/\bMuju\b/g,'Moo-joo').replace(/\bHono\b/g,'HOH-noh').replace(/\bKagari\b/g,'kah-GAH-ree').replace(/\bRadi\b/g,'RAH-dee').replace(/Umeme/g,'oo-MEH-meh').replace(/Kimubunga/g,'kee-moo-BOON-gah').replace(/Aegirinn/g,'AY-gear-in').replace(/Sachakuna/g,'sah-chah-KOO-nah').replace(/Sachita/g,'sah-CHEE-tah').replace(/Karanlık/g,'kah-rahn-luk').replace(/Gölge/g,'GURL-geh').replace(/Göl/g,'Gurl').replace(/\bStraumr\b/g,'STROWM-ur').replace(/\bYan\b/g,'Yahn').replace(/\bMazask\b/g,'MAH-zahsk').replace(/\bTanka\b/g,'Tahn-kah').replace(/\b([A-J]) (one|two|three|four|five|six|seven|eight|nine|ten)\b/g,(_,c,n)=>({A:'Ay',B:'Bee',C:'See',D:'Dee',E:'Ee',F:'Eff',G:'Gee',H:'Aitch',I:'Eye',J:'Jay'}[c]+'-'+n));
...
async function make(line){
 ...
 if(line.cue&&!cueMap[line.cue])throw Error(`Unknown delivery cue ${line.cue}`);const input=pronounce(line.text).replace(/\b(Muju|Hono|Kagari|Radi|Umeme|Kimubunga|Straumr|Aegirinn|Gölge|Göl|Karanlık|Sachita|Sachakuna|Yan|Mazask|Tanka)\b/gu,n=>({Muju:'Moo-joo',Hono:'HOH-noh',Kagari:'kah-GAH-ree',Radi:'RAH-dee',Umeme:'oo-MEH-meh',Kimubunga:'kee-moo-BOONG-gah',Straumr:'STROWM-ur',Aegirinn:'AY-geer-in',Göl:'GUHL',Gölge:'GUHL-geh',Karanlık:'kah-rahn-LUK',Sachita:'sah-CHEE-tah',Sachakuna:'sah-chah-KOO-nah',Yan:'Yahn',Mazask:'MAH-zahsk',Tanka:'Tahn-kah'}[n]));const body={model,voice:episode.voices[line.speaker],input,instructions:[directions[line.speaker],cueMap[line.cue]||'',line.performanceNote||'',tail].join(' '),response_format:'wav'};
