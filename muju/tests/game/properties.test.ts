...
for(const budget of [4] as const) for(let seed=1;seed<=20;seed++)it(`${budget}-action full-state invariants and reducer/search equality, seed ${seed}`,()=>{
 ...
 for(let ply=0;ply<500&&state.phase==='playing';ply++){
  ...
  // Direct UI moves retain selection; compare the full game state after clearing presentation.
  expect(gameReducer(gameReducer(before,action),{type:'DESELECT'})).toEqual(state);checkInvariants(state,`${seed}/${ply}`);
