...
for(const width of [390,834]) {
 test(`upkeep choice and public rent at ${width}px`,async({page},info)=>{
  ...
  await expect(page.locator('.progress-clock')).toContainText('7/10 turns without a kill');await expect(page.locator('.progress-clock')).toContainText('paid 1 · released 1');await expect(page.locator('.score-strip')).toContainText('Upkeep 1');
 ...
 test(`inactivity draw reaches victory screen at ${width}px`,async({page},info)=>{
  ...
  await page.addInitScript(({state,schemaVersion})=>localStorage.setItem('elemental-tactics-save',JSON.stringify({schemaVersion,timestamp:Date.now(),state})),{state,schemaVersion:SCHEMA_VERSION});
  await page.goto('./');await page.getByRole('button',{name:'Pass & Play'}).click();await page.getByRole('button',{name:/Continue saved game/}).click();
  await page.getByRole('button',{name:'End turn →',exact:true}).click();await expect(page.getByRole('heading',{name:'Draw by inactivity'})).toBeVisible();await expect(page.getByText('10 consecutive player turns passed without a kill. Crystal income does not reset the clock.')).toBeVisible();
