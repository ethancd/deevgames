...
it('mass purchases use unique deterministic IDs identical in reducer and search',()=>{
 let s=createInitialGameState(undefined, 4, 0, 'phasing');s.turn.phase='place';s.players.white.resources=100;s.players.white.resourcesGained=100;s.board.units[1].position={x:7,y:7};
 ...
 expect(new Set(s.board.units.map(u=>u.id)).size).toBe(s.board.units.length);expect(s.players.white.resources).toBe(10);
});
