...
const ruleVariant=(process.argv[4]??'shipped') as 'off'|'shipped';
if(!['off','shipped'].includes(ruleVariant))throw Error('Invalid variant');
...
try{for(let cell=0;cell<pairs.length;cell++){
 ...
 for(let i=0;i<n;i++)for(const swapped of [false,true]){
  ...
  games.push({maxInactivityPlies:r.maxInactivityPlies,inactivityDraw:r.inactivityDraw,upkeepElimination:r.upkeepElimination,purchases,players:r.players,a,b,seed,swapped,winner:r.winner,winType:r.winType,turns:r.turns,plies:r.plies,cap,illegal:r.players.white.illegalActions+r.players.black.illegalActions,invariant:r.invariantViolation,anomalies:r.anomalies});
 }
