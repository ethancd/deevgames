...
def plan(graph, by_id, ordered, kinds=(), node_ids=(), files=()):
    ...
    return {
        ...
        'nodes': [{**by_id[node_id], 'status': 'pending', 'reasons': reasons[node_id],
...
def markdown(result):
    ...
    for index, node in enumerate(result['nodes'], 1):
        ...
        lines += ['', 'Disposition: pending. Evidence: —', '']
    if not result['nodes']:
...
def main():
    ...
    commands.add_parser('check', help='Validate graph, acyclicity and local paths; does not test the product')
    ...
    try:
        ...
        by_id, ordered = validate(graph)
        if args.command == 'check':
            print(f'Valid DAG: {len(by_id)} nodes, {sum(len(n["depends_on"]) for n in graph["nodes"])} edges; all declared local paths exist.')
