...
it('spending the last three crystals finishes placement, permits haste and is undoable until turn end',()=>{
 const s=createInitialGameState();s.turn.phase='place';s.players.white.resources=3;s.players.white.resourcesGained=3;saveGameState(s);
 const {result,unmount}=renderHook(()=>useGameState());act(()=>result.current.buyUnit('fire_1',{x:0,y:0}));
 expect(result.current.state.turn).toMatchObject({phase:'action',currentPlayer:'white',actionsRemaining:4});expect(result.current.canUndo).toBe(true);
