...
| `muju_observe` | `killClock.plies` is within three of `killClock.limit` (7 of 10) | The remaining kill-free hand-offs end the game on mined totals, not a draw. Check `killClock.leader`: if it is you, keep it quiet; if it is the opponent or `null`, spend the turn on a kill — only an attack that removes a unit resets the clock, to zero, and income, movement, purchases, promotions and upkeep losses never do. Read the numbers from the observation instead of counting turns yourself; the rule moved from a twenty-ply draw to this ten-ply mined-total verdict on 2026-09-22 (rules revision `muju-phasing-3`). |
...
| You are choosing a `BUY_UNIT` square | A spawn rectangle runs from your home corner (White `A1`, Black `J10`) to one of your units, inclusive, and any enemy unit inside it blocks it. Two tests apply. At **commit** time the square must already be empty, inside one of your current unblocked spawn rectangles, and free of another own commitment, or the action is illegal — and the batch is atomic, so it takes your whole turn with it. Read the squares from `briefing.sections.spawn` or `muju_legal_actions`; never copy a square from an example. (At the opening, White's `B1` holds White's own Hi.) |
