...
- Prompt priority (wave 2 onwards): (1) don't lose on the wall clock, (2) don't lose on the kill clock, (3) try to win, (4) the investigation brief, which never overrides 1–3 and is rendered after them. The wall-clock section asks for measured, not felt, time: a per-turn cap of the free delay plus a tenth of the remaining bank, `muju_clock` checkpoints before expensive steps, `clockPressure` calibration, lean context (one `sinceRevision` briefing per turn) and a `muju_stage` safety net above `bare`. Clocks are not raised per model.
...
A ticket sets `"engineProfile": "strategos"` (any `hardConfigFor` label from `lab/hard-ai/bots/hard.ts`; `env` and empty labels are refused); a wave sets a default for every ticket without one with a top-level `"engine": { "profile": "strategos" }` in `wave.json`, next to its `sourceSha256`/`rulesId`/`note`, and a ticket's own `"engineProfile": "desktop"` opts back out. Both are checked with the engine seat's own rules (`pilot.ts#validateEngineProfile`) when the schedule loads, so a bad label fails `--dry-run` or the first tick before any room exists, and a game that has started keeps the profile it started with even if the wave is edited later. A non-desktop game names the engine seat "Hard (<profile>)" in the room, writes `profile` into the engine seat's config, and records the profile in `manifest.json` (`engine.name`, `engine.profile`), in the reflection's engine identity and in its published experience record (`engineProfile`); a desktop game's room name, engine config, manifest and experience record are byte-identical to those of a game from before this knob.
