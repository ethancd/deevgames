...
import { AI_PROTOCOL, sameRequest, type EngineUsed, type SearchProgress, type SearchRequest, type SearchResponse, type TurnResult } from './protocol';
...
export interface FindTurnResult {
  ...
  /** Which engine actually answered (E5.1). A responder that predates the
   * field means `'v2'`, so `undefined` is normalized away here. */
...
// `engineUsed` absent ≡ `'v2'` (protocol.ts), the same default on both wire
// shapes: a responder that predates the field was never the hard engine.
function fromTurnResult(result: TurnResult, engineUsed: EngineUsed = 'v2'): FindTurnResult {
  return {
    actions: result.actions, scoreCc: result.scoreCc, depth: result.depth, work: result.work,
    source: result.source, endKey: result.endKey, fallback: result.fallback, engineUsed,
  ...
  async findBestTurn(state: GameState, difficulty: AIDifficulty, decisionMs: number, revision: number, options: FindTurnOptions = {}): Promise<FindTurnResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.worker!.onmessage = ({ data }) => {
        ...
        resolve(data.type === 'turn' ? fromTurnResult(data.result, data.engineUsed) : fromAIResult(data.result, data.engineUsed));
      };
      this.worker!.onerror = (event) => { finish(); this.cancel(); reject(new Error(event.message || 'AI worker failed')); };
      this.timer = setTimeout(() => { finish(); this.cancel(); reject(new Error('AI worker timed out. Please retry.')); }, Math.max(2000, decisionMs + 2000));
      this.worker!.postMessage(request);
