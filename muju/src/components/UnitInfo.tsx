...
export function UnitInfo({ phasingIn, unit, previewDefinitionId, cellInfo, isPlacePhase, isActionPhase, resources = 0, onPromote, isEnemyView, inspectOnly, onClose, currentPlayer, showEnemyRange, onToggleEnemyRange, showNextTier, actionsRemaining }: UnitInfoProps) {
  ...
  return <div className="unit-detail">
    <div className="unit-heading"><strong><UnitArtwork element={def.element} owner={unit?.owner ?? currentPlayer ?? 'white'} tier={def.tier} /> {def.name} <small>{def.element} · T{def.tier}{phasingIn ? ' · Phasing in' : isEnemyView ? ' · Enemy' : ''} · Upkeep {upkeepForTier(def.tier)}</small></strong><button aria-label="Deselect unit" onClick={onClose}>×</button></div>
