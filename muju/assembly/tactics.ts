...
function possible(actions: i32): bool {
  ...
  for (let u = 0; u < count; u++) {
    ...
    const speed = catalogue[def(u) * 4 + 2];
...
function dfs(actions: i32, depth: i32): bool {
  ...
  for (let u = 0; u < count; u++) {
    ...
    const start = pos(u), speed = catalogue[def(u) * 4 + 2], base = depth * 100;
    if (speed <= 0) continue;
