...
export function tacticalFixtures(): TacticalFixture[] {
  ...
  const rotation = fixture('five-action rotation exceeds the turn budget', [['water_3',1,0],['water_3',0,1],['water_3',2,0]], 'metal_3', 4);
  rotation.expected = 'disproved';
  const promo = fixture('promotion dependent rescue', [['shadow_2',1,0],['shadow_2',0,1]], 'metal_3', 4, 24, true);
  const noMoney = fixture('public bank / no promotion money', [['shadow_2',1,0],['shadow_2',0,1]], 'metal_3', 4, 0, true); noMoney.expected = 'disproved';
  ...
  const attacked = fixture('same-target limit', [['fire_2',1,0],['fire_2',0,1]], 'metal_3', 4); attacked.state.board.units[1].attackedThisTurn = ['invader']; attacked.expected = 'disproved';
  const chip = fixture('damage remains during defender reply', [['fire_2',1,0]], 'metal_3', 1); chip.state.board.units[0].damageTaken = 2;
  const placed = fixture('newly placed unit cannot promote', [['shadow_2',1,0],['shadow_2',0,1]], 'metal_3', 4, 24, true); placed.state.board.units.slice(1).forEach(u => u.placedThisTurn = true); placed.expected = 'disproved';
  ...
  const already = fixture('at most one promotion', [['shadow_2',1,0],['shadow_2',0,1]], 'metal_3', 4, 24, true); already.state.board.units.slice(1).forEach(u => u.promotedThisPlacement = true); already.expected = 'disproved';
