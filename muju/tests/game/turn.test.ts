...
describe('two-phase boundary order',()=>{
 ...
 it('ends at exactly ten completed kill-free player turns, tied on mined totals',()=>{
  ...
  // A symmetric board with no attacks gives both sides equal passive income.
  expect(getGameResult(s)).toEqual({status:'draw',reason:'kill-clock'});expect(endTurn(s)).toBe(s);
 });
 it('collects before the kill clock, and it decides before the next home win or upkeep',()=>{
  ...
  const ended=endTurn(s);expect(ended.victoryReason).toBe('kill-clock');expect(ended.winner).toBeNull();expect(ended.lastIncome).toMatchObject({player:'black',total:0});expect(ended.lastUpkeep).toBeUndefined();expect(ended.board.units).toEqual(s.board.units);
  s.inactivityPlies=INACTIVITY_LIMIT-2;expect(endTurn(s).victoryReason).toBe('home-occupation');
  s.inactivityPlies=INACTIVITY_LIMIT-1;s.board.cells[9][8].resourceLayers=1;expect(endTurn(s).victoryReason).toBe('kill-clock');
 });
