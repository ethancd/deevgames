...
| 5 | Full `npx vitest run` green | **PASS**, saved full run: 138 files / 1,916 tests; later affected Hard run: 42 files / 530 including the added case |
...
### 2.6.5 KEYS: the MATE-only argument and its scope

Order-distinct states share `Kpos`/`Kturn`. Canonical order is carried and
restored as replica state, while these keys continue to describe semantic board
state. The current Phasing home-checkmate decision does not require adding
canonical sequence order to the keys:

1. Phasing runs the act-only DFS. Every recursive MOVE or ATTACK reduces AP,
   which is included in the failure key; a key cannot recur on the active stack.
2. On a **no-rescue** graph, let N be the number of distinct expandable keys
   after terminal and damage-bound pruning. A completed failed subtree is
   memoized, so each such key is charged exactly once. Traversal order changes
   their visit order, not N. MATE is returned exactly when N <= the node cap.
3. A graph containing a rescue can return RESCUE or UNKNOWN, but cannot return
   MATE. `make` and canonical adjudication consume only `verdict === MATE`.
   Thus their home-checkmate terminal decision is order-independent.

This reasoning assumes order-independent successor sets and bounds, a key that
represents the state needed by them, and the packed failure memo's ordinary
64-bit collision assumption. It applies to current Phasing act-only DFS, not
Standard preparation, arbitrary interruption or a future proof-number solver.
It establishes semantic terminal equivalence, not byte-identical order planes,
slot mappings, witnesses or search traces. The per-call failure set is reset;
there is no cross-invocation failure-memo reuse.

**Independent review corrected an earlier broader objection.** Codex's initial
proposal required an order signature before all semantic TT hits. After reviewing
the argument above, both reviewers withdrew that blanket requirement for this
MATE-only path. Different rescue witnesses alone do not prove different game
values or an incorrect cached score; public keys can remain unchanged.

**Witnesses and bounded search remain a separate M4 obligation.** `homeWitness`
feeds a candidate line into the generator, including when the proof is uncapped.
Two legally reached purchase-order variants independently produced RESCUE with
4 nodes/4 actions versus 3 nodes/3 actions at cap 20,000; packed and canonical
agreed separately. Therefore zero cutoff telemetry does not establish identical
witnesses, candidate prefixes or fixed-work traces. M4 must state and verify its
selection/reproducibility contract; this observation is not itself a bad-TT-score
reproduction. Ranked keep-set prefixes can also differ below the full legality
capacity of 64: the interior generator selects only its first four candidates.

**Slot-bound table reuse is another distinct M4 obligation.** `NodeTables`
memoizes by square-keyed Kturn/catalog but returns slot-indexed arrays. Reuse
across different slot mappings needs a mapping guard, remap/rebuild, or an
enforced caller invariant before the hit. The MATE theorem does not validate
stale slot-indexed buffers. Changing only ordinal ranks with slots held fixed is
not the same issue. Exact proof-cache semantics must be assessed separately when
its currently inactive df-pn implementation becomes active.

Reversing birth order on 1,038,541 measured production-cap positions produced no
MATE-classification or mate-node-count violations. Reversal is one permutation
check, not exhaustive permutation coverage; the stated graph argument carries
the universal claim. `cappedProverCalls` records only full gate proofs inside
`make` that exhaust their node cap. It remains useful exposure telemetry and a
release tripwire, not a certificate for all cache or selective-search behavior.
...
| p99 / p99.9 upper bounds | **<= 7 / <= 15** nodes |
...
a million, and `gateProverCapHits` is 0 on every one of the 26 seeds. This
measures cap headroom on the sampled positions. Re-proving them with reversed
birth order also found no MATE/non-MATE disagreement. Neither observation proves
order-independent witnesses, bounded generator selection, or slot-indexed cache
reuse; those remain separate M4 contracts. The universal MATE-only argument and
its assumptions are stated in §2.6.5.
...
| `cappedProverCalls` / `gateProverCapHits` | inside-make full-proof cap telemetry, also published as `HardSearchStats.cappedProverCalls`; not a witness/selection/cache-reuse certificate |
...
observed in a million, the recorded bucket bounds are p99 <= 7 nodes and p99.9 <= 15. `gateProverCapHits` is 0 on
every seed, so no production-cap gate exhaustion was observed. The separate MATE
argument in §2.6.5, rather than this sample, establishes its order invariance.

**Artifacts.** The 26 per-seed artifacts are retained externally, including
coverage counts, distributions, timestamps and the zero-error counters. Codex's
independent audit copied and hashed them under its `work/m2-round6-independent-review/`
directory. They carry `gitDirtyFiles`/`treeMatchesCommit` provenance, not execution
source hashes; review-time snapshots cannot retroactively supply those hashes.

Integration review corrected this gate-only histogram: the previous aggregate
included 206 arrival-surface observations. The corrected entries sum to
1,014,307. Primary action/comparison/error totals and maximum 385 were unchanged.
The histogram supports percentile upper bounds, not exact within-bucket values.
...
```
npx vitest run       ->  138 files, 1,916 tests, 0 failures (saved 03:26 full log)
npm run hard:test    ->   42 files,   530 tests, 0 failures (later affected run, including added case)
```
