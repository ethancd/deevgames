...
const n=Number(process.argv[2]??30),shard=Number(process.argv[3]??0),shards=Number(process.argv[4]??4);
const pairs=[['Invade:LightningRush','Guard:InvestT3'],['Invade:LightningRush','Guard:Balanced'],['Siege:4','Guard:Balanced'],['Invade:InvestT3','Guard:InvestT3']];
if(!Number.isInteger(n)||n<1||shard<0||shard>=shards)throw Error('Invalid args');
const out='lab/results/home-guard-2026-09-07';mkdirSync(out,{recursive:true});
...
writeFileSync(`${out}/manifest-${shard}.json`,JSON.stringify({n,shard,shards,pairs,started:new Date().toISOString(),sourceHash,unitHash:createHash('sha256').update(JSON.stringify(UNIT_DEFINITIONS)).digest('hex'),seedBase:9071426,maxTurns:120,notes:'Same policies/seeds/seats, map D, v1.3 units. New-objective probes use bounded public-state home defense on both rule variants.'},null,2));
