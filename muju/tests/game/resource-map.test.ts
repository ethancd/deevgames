...
describe('Unequal routes passive reserves',()=>{
 it('starts new games on the central-reserve map with 480 total',()=>{
  ...
  paint(['A1','B1','C1','A2','B2','A3','J8','I9','J9','H10','I10','J10','H2','I2','H3','I3','B8','C8','B9','C9'],10);
  paint(['F4','D5','E5','F5','E6','F6','G6','E7'],8);
  expect(UNEQUAL_ROUTES_MAP).toEqual(expected);
  expect([...UNEQUAL_ROUTES_MAP].reverse()).toEqual(UNEQUAL_ROUTES_MAP);expect(INITIAL_MAP_RESOURCES).toBe(480);
  expect([0,4,8,10].map(n=>UNEQUAL_ROUTES_MAP.filter(x=>x===n).length)).toEqual([18,54,8,20]);
 ...
 it.each([5,6])('keeps the old map and depletion when resuming a schema-%s save',schemaVersion=>{
  ...
  expect(resumed.board.initialResourceLayers).toEqual(PRE_CENTRAL_MAP);
  ...
  expect(next.board.initialResourceLayers).toEqual(PRE_CENTRAL_MAP);
