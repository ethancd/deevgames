...
before it lands: per plan B.1/B.2 step W1.10, after `iterativeDeepening` completes, `search/root.ts` will pick
the best PLAN-CONSISTENT root candidate instead of the search's own top-scored one, UNLESS the searched score
is terminal-scale worse than the tactical best (a proof — mate, or a proven clock loss) or the plan's contract
loses an essential slot in the opponent's first reply. Ordinary material loss is never a veto reason (Part A
item 2: the search's job is to falsify the plan's CONTRACT, not to protect material). This is a genuine
deviation from "the root plays its highest-scored searched candidate" — the rule every other profile keeps —
scoped to `SearchFix.strategyVeto`, `hard@strategos` only. This entry will be replaced with the shipped
mechanism, its commit(s) and what review found once W1.9/W1.10 land.
