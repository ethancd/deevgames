  ...
  return <div className="unit-detail">
    ...
    <div className="unit-action-row">
      ...
      : isPlacePhase && unit.owner === currentPlayer ? <>
        <p>{!next ? 'Maximum tier · terminal' : unit.placedThisTurn ? 'Newly placed · upgrade next turn' : unit.promotedThisPlacement ? 'Already upgraded this placement' : next ? `${next.name}: ATK ${next.attack} · DEF ${next.defense} · SPD ${next.speed} · MINE ${next.mining}` : 'Maximum tier'}</p>
        {next && <button onClick={onPromote} disabled={!upgrade}>Upgrade · ◆ {cost} · rent {upkeepForTier(next.tier)}</button>}
