/**
 * E4.3 candidate A: `lab/results/hard-ai-e3/p8/repro-turn.ts` copied verbatim
 * except for this header and the import depth, so the before/after table of
 * `docs/hard-ai/e4/E4.3-RESCUE-CAP.md` is measured by the same instrument that
 * measured P8. Diagnostic only; imports nothing into the engine.
 *
 * `--engine` is the ladder engine name with `hard@` optional, so the arm is
 * reached as `--engine ablate:search-rescue-cap` and the champion as
 * `--engine desktop`; no flag plumbing of its own.
 *
 * node --import tsx --cpu-prof --cpu-prof-dir=<dir> repro-turn.ts \
