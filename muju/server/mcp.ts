...
export function createMcpServer(backend: RoomBackend, publicUrl: string) {
  ...
  server.registerTool('muju_rules', { description: 'Read rules for the chosen ruleset, unit stats, coordinates, and workflow before playing. Pass the room ruleset; defaults to standard. Phasing has actions first, end-of-action mining/upkeep, public delayed summons and END_PLACE_PHASE handoff.',
    inputSchema: { ruleset: z.enum(['standard', 'phasing']).default('standard') }, annotations: readOnly },
    async ({ ruleset }) => output(rulesFor(ruleset)));
  server.registerTool('muju_create_room', { description: 'Host a new two-player game. Optional immutable matchPolicy declares an experiment toolTier and protocolId for the entire room; omitted ordinary rooms retain all tools. ruleset defaults to standard; choose phasing for public delayed summons and end-of-turn upkeep/promotions. In Phasing both players start with actions regardless of handicap. Optional blackCrystalHandicap grants Black 1–20 starting crystals (omit/0 for none). In Standard, Black skips its first Place & Promote phase with 1–2 crystals; 3–20 enables it. White still starts. Optional immutable timeControl: blitz (10s/2min), rapid (30s/10min), classical (60s/30min), or custom delaySeconds/bankSeconds. Each player has a separate bank; free delay resets each full turn. Omit for untimed. Clocks start when the opponent joins. Returns your private seat credential and a separate invitation to share.', inputSchema: createSchema.shape,
  ...
  server.registerTool('muju_preview', { description: 'Simulate an atomic sequence without committing requested moves. Returns a hypothetical board and separate liveClock, liveClockPressure and private liveStaging for the real game. Thinking and previews use your running clock; they never reserve time or reset delay. The room service still resolves due stages/expiry before a preview; re-read if that changed the revision. An expired clock returns TIME_EXPIRED with the actual terminal room. Use A1–J10 positions. A batch may end the full turn but cannot act as the opponent. In Phasing include END_PLACE_PHASE after actions/mining/upkeep/preparation for a complete-turn preview.',
  ...
  server.registerTool('muju_play', { description: 'Commit one action or an atomic sequence using current revision and unique requestId. Invalid requested batches change nothing. An immediate home checkmate cancels its queued tail. Standard END_ACTION_PHASE hands over; Phasing END_ACTION_PHASE mines and pays upkeep, then END_PLACE_PHASE hands over after summoning/promotions. All phases/actions share one delay; late commands return TIME_EXPIRED without requested actions. RESIGN concedes. UNDO alone reverses your latest command this turn when canUndo is true, including automatic upkeep. The service settles due stages/expiry first, even if your request then fails. Timed play/undo includes your private staging status: partial commands leave pending work intact; handoff or result clears it. Recheck plans after live play/undo. Retry identical requests after uncertain outcomes.',
