...
const variant=(process.argv[5]??'shipped') as 'off'|'shipped'|'steep';
if(!['off','shipped','steep'].includes(variant))throw Error('Invalid variant');
...
for(let cell=0;cell<pairs.length;cell++){
 ...
 for(let i=0;i<n;i++)for(const swapped of [false,true])for(const rule of ['home-or-elimination'] as const){
  ...
  const {record:r,replay}=await playGame({bots:{white:makeHomeBot(swapped?b:a),black:makeHomeBot(swapped?a:b)},seed:deriveSeed(9071326+cell,i),engineHash:sourceHash,runId:`upkeep-${variant}`,experiment:rule,options:{upkeep:variant,inactivityRule:variant==='off'?'off':'on',victoryRule:rule,legality:'strict',maxTurns:120,checkInvariants:true,recordReplay:false},onAction(before,after){
