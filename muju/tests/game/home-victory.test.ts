...
/** Phasing is the only ruleset since 2026-09-21: a turn ends with END_PLACE_PHASE. */
const phasing=()=>createInitialGameState(undefined,4,0,'phasing');
const endWholeTurn=(s:ReturnType<typeof phasing>)=>applyAction(applyAction(s,{type:'END_ACTION_PHASE'}),{type:'END_PLACE_PHASE'});
...
for(const attacker of ['white','black'] as PlayerId[])describe(`home victory ${attacker}`,()=>{
 ...
 function invasion(){const s=phasing();s.turn.currentPlayer=attacker;s.board.units=s.board.units.filter(u=>u.owner===attacker||!(u.position.x===k&&u.position.y===k+(k?-1:1)));const u=s.board.units.find(u=>u.owner===attacker&&u.definitionId==='fire_1')!;u.position={x:k,y:k+(k?-1:1)};return {s,u};}
 it('arrival is not victory; opponent gets a full reply, then turn start wins before upkeep/healing',()=>{
  ...
  next=endWholeTurn(next);expect(next.turn.currentPlayer).toBe(defender);expect(next.phase).toBe('playing');
  expect(getAllSpawnPositions(defender,next.board)).toHaveLength(0);
  const result=endWholeTurn(next);expect(result.winner).toBe(attacker);expect(result.victoryReason).toBe('home-occupation');
  ...
  expect(applyAction(result,{type:'RESIGN'})).toBe(result);
  expect(gameReducer(gameReducer(next,{type:'END_ACTION_PHASE'}),{type:'END_PLACE_PHASE'})).toEqual(result);
 });
 it('killing the occupier during the reply prevents victory',()=>{
  ...
  next=endWholeTurn(next);expect(next.phase).toBe('playing');expect(next.turn.currentPlayer).toBe(attacker);
 });
 it('zero-attack units qualify; friendly home occupation does not; legacy comparison is explicit',()=>{
  const s=phasing();const u=s.board.units.find(u=>u.owner===attacker&&u.definitionId==='plant_1')!;u.position={x:k,y:k};
 ...
 it('loading an invasion mid-turn is not a retroactive victory; terminal saves retain the reason',()=>{
  ...
  next=endWholeTurn(next);saveGameState(next);expect(loadGameState()?.victoryReason).toBe('home-occupation');
 });
});
it('simultaneous invasions resolve for the player whose turn starts first',()=>{
 const s=phasing();s.board.units.find(u=>u.owner==='white')!.position={x:9,y:9};s.board.units.find(u=>u.owner==='black')!.position={x:0,y:0};
...
it('the AI does not resign a material deficit under the invasion rule',()=>{
 const s=phasing();s.board.units=s.board.units.filter(u=>u.owner==='black'||u.definitionId==='fire_1');
 expect(shouldResign(s,'white')).toBe(false);s.victoryRule='elimination';expect(shouldResign(s,'white')).toBe(true);
});
it('a five-action three-attacker rotation cannot clear a corner in four actions',async()=>{
 const {clearHomePlan}=await import('../../lab/experiments/home-policies');
 let s=phasing();s.turn.currentPlayer='black';s.board.units=[createUnit('metal_3','white',{x:9,y:9}),createUnit('water_3','black',{x:9,y:8}),createUnit('water_3','black',{x:8,y:9}),createUnit('water_3','black',{x:8,y:8})];
 const plan=clearHomePlan(s);expect(plan).toBeNull();
 expect(getHomeOccupier(s.board,'white')).toBeDefined();
});
