...
it('reserves unique lowercase codes across restarts, keeps them private and consumes invitations once', () => {
  ...
  expect(() => store.join(host.room.id, { name: 'Third', inviteCode: host.inviteCode })).toThrow('already been used');
  // Used links can still locate the room so browsers can restore their own saved seats.
  expect(store.resolveInvitation(host.inviteCode!)).toEqual({ roomId: host.room.id });
  expect(store.restore(host.room.id, guest.credentials.token, guest.credentials.player).ready).toBe(true);
