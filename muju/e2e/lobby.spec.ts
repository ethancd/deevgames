...
test('phone lobby discovers games, watches with one tap, and refreshes after a result', async ({ browser, request }, testInfo) => {
  ...
  // (the default config's, which nothing tears down between runs) still holds
  // the rooms from last time, and a repeated name matches two cards.
  const run = Math.random().toString(36).slice(2, 8);
  const hosted = await (await request.post('/api/muju/rooms', { data: { name: `Lobby White ${run}` } })).json();
  const roomId = hosted.room.id;
  const guest = await (await request.post(`/api/muju/rooms/${roomId}/join`, { data: { name: `Lobby Black ${run}`, inviteCode: hosted.inviteCode } })).json();
  try {
    ...
    const watch = lobby.getByRole('button', { name: `Watch Lobby White ${run} vs Lobby Black ${run}`, exact: true });
    ...
    await expect(phone.getByRole('heading', { name: `Lobby White ${run} Wins!` })).toBeVisible();
    ...
    const next = await (await request.post('/api/muju/rooms', { data: { name: `New lobby game ${run}` } })).json();
    const nextCard = lobby.getByRole('listitem').filter({ hasText: `New lobby game ${run}` });
    await expect(nextCard).toContainText('Waiting for opponent', { timeout: 15000 });
    await request.post(`/api/muju/rooms/${next.room.id}/join`, { data: { name: `New opponent ${run}`, inviteCode: next.inviteCode } });
