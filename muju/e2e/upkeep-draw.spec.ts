...
for(const width of [390,834]) {
 test(`upkeep choice and public rent at ${width}px`,async({page},info)=>{
  ...
  const base=createInitialGameState();base.board.units[0].definitionId='fire_2';base.board.units[1].definitionId='water_3';base.players.white.resources=1;base.players.white.resourcesGained=1;base.inactivityPlies=16;
 ...
 test(`inactivity draw reaches victory screen at ${width}px`,async({page},info)=>{
  await page.setViewportSize({width,height:width===390?844:1112});
  const state=createInitialGameState();state.inactivityPlies=19;state.turn.phase='queue';
  await page.addInitScript(({state,schemaVersion})=>localStorage.setItem('elemental-tactics-save',JSON.stringify({schemaVersion,timestamp:Date.now(),state})),{state,schemaVersion:SCHEMA_VERSION});
  await page.goto('./');await page.getByRole('button',{name:'Pass & Play'}).click();await page.getByRole('button',{name:'Start Game'}).click();
  await page.getByRole('button',{name:'End turn →',exact:true}).click();await expect(page.getByRole('heading',{name:'Draw by inactivity'})).toBeVisible();await expect(page.getByText('20 turns passed without mining crystals or eliminating an enemy by attack.')).toBeVisible();
