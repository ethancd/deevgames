...
## 2026-09-21 Phasing-only cutover + AI campaign (session deevgames-81)
- State found: main checkout /Users/ashkie/src/deevgames sits on codex/muju-local-wip-2026-09-16 with 244 dirty files, 105 commits BEHIND origin/master. Real work is on origin/master (eda73b26, PR #25 Hard-AI Phasing repair merged 2026-09-21). Never edit the main checkout for Muju; work in worktree /Users/ashkie/src/deevgames-phasing-cutover (branch claude/phasing-only-cutover from origin/master).
- "Next session" notes = muju/docs/hard-ai/phasing/repair-2026-09-20/HANDOFF.md (+ reports/understand/phasing-only-plan.md). CLAUDE.md on origin/master points there; the local CLAUDE.md copy is stale.
- CI (Deploy Games to Cloudflare Pages) has been red on every master push since 2026-09-19: PR #25 changed DEFAULT_WEIGHTS without re-pinning tests (7 vitest files: tests/ai/hard/{eval,phasing-bootstrap}, tests/lab/{ablate,phasing-tune-guards,gate1,gate1-shard,suites-phasing-contract}). Render still auto-deploys master regardless (no test gate). Pages has no CLOUDFLARE_API_TOKEN secret; local deploy uses the wrangler OAuth login (pages:write) + `npx wrangler pages deploy _site --project-name deevgames --branch master` after `bash build-all.sh`.
- Prior Codex cutover attempt codex/phasing-only-canonical (2 commits, 199 files) is 65 commits stale and introduced revision string muju-online-5 — reference only; keep muju-phasing-2.
- Academy media bundle IS present at ~/Archives/muju-media-2026-09-18; narration = OpenAI TTS (paid, needs explicit owner authorization per academy/README). Text-only Phasing notice is prepared in docs/changes/t7-academy-notice-2026-09-19/website-notice.patch.
- Campaign layout (2026-09-21): plan + seven reports in the session scratchpad w1/ (PLAN.md is the authority: decisions D1–D14, lane ownership, stage order). Stage-1 lanes run in worktrees ~/src/deevgames-pc-lane{1,2,3,5,6} (branches claude/pc-laneN): L1 ci-green, L2 ruleset-core, L3 persistence-archive, L5 docs, L6 ai-strength (knobs default-off). Merge order into claude/phasing-only-cutover: L1 → L3 → L2 → L5 → L6; known touch points tests/lab/ablate.test.ts:198 and lab/hard-ai/ablate/arms.ts. Stage 2 = tests/e2e re-point on the merged branch (lane4 worktree); Stage 3 = full validation + Gate-0 informational suite measure; Stage 5 = PR → merge (Render deploy) → local wrangler Pages deploy → Academy notice → completion record + standard-final re-point.
- Production baseline captured BEFORE any deploy: scratchpad w1/prod-before/ (0 active rooms; 41 archived = 28 standard + 13 phasing; 12×200 / 111×409 per-room probes). Re-run the same probe after deploy and diff; any changed archivedAt or status code is a failure.
- Stage 1 merged 2026-09-21 ~21:15 PDT into claude/phasing-only-cutover @ 08b98a16 (L1→L3→L2→L5→L6, zero conflicts). Lane-ownership discipline worked: 24 agents, 5 worktrees, the only cross-lane surprises were files NO lane owned (tests/lab/texel.test.ts, tests/server/mcp.test.ts) — always assign every file a lane, including "nobody's" test files. Reviewer catches worth remembering: an fs walker replacing `rg --files` must skip dotfiles/gitignored paths; a shared scorer helper changed one lab arm's play (hard@lab-refined, useFutility) with an UNMOVED config hash — identity hashes only cover config, not code paths; "Reset analysis" on the retired-save review screen seeded a fresh Standard board (residual way to play retired rules).
- Stage 2 (2026-09-21 ~21:25 PDT) lanes on worktrees lane4 (S2A tests re-point), lane7 (S2B e2e+smoke), lane8 (S2C docs residual), lane9 (S2D UI residual + phone AI profile), all from 08b98a16; handoffs in scratchpad w3/.
- Stage 2 merged 2026-09-21 ~23:20 PDT → claude/phasing-only-cutover @ d68db689 (one trivial conflict in tests/hooks/ai-turn-time.test.tsx, took lane9's). Stage 3 (validation on the cutover worktree, measurement on lane1 which may push its branch, records on lane8, Fable ship verdict) launched ~23:30. Stage-2 learnings: F1 "flip board.ts default" is really a CODE-DELETION job (Standard branches in turn.ts/homeCheckmate.ts/simulate.ts/turnFunding.ts/server/analysis + lab audit remain, covered only by 61 defaulted test call sites); F8: a Phasing turn planned in one search spends only 75% of the pace allowance (turnFunding reserves 2/8 for Prepare, never requested); phones ran DESKTOP tables until S2D (profileFor was never called).
- Stage 3 done 2026-09-22 ~01:20 PDT: full validation GREEN (vitest 2944/2944, online e2e 84, default e2e 111, hard e2e 12, ai:tactics 36/36, balance:check, build-all + smoke, perft canonical+replica, fuzz 20k actions 0/0/0, retirement smoke 5/5); identity bridge base eda73b26 vs tip IDENTICAL move-for-move (hard@desktop unaffected by any lane); Fable verdict SHIP, zero blocking, conditional on a green CI dispatch. Gaps found: Gate-0 suite measure refused because the v2 manifest pins package.json bytes and L1 deleted two dead scripts (lesson: a release-suite manifest pinning package.json makes ANY script edit void the suite — add a live-tree manifest test); hard:determinism broken since e701ccc0 (determinism.ts uses hardConfigFor → placeholder weights); hard:verify M1 pins 11 fixtures vs 7 Phasing; M6–M13 need Standard corpora (follow-up). Three of four L6 strength knobs never change a chosen move at fixed:50000 (R1b/R2/R3 inert; only R4 at-risk share moves play, +0.156 vs Rush on 16 pairs). Control vs Rush at fixed:50000 is 6–0–26 (the handoff's 12–0–20 was a wall:1500 row).
