...
export function selectChild(node: MCTSNode, exploration: number, alpha: number, valueSign = 1): MCTSChild | null {
  ...
  for (const child of children) {
    const exploit = valueSign * child.node.totalValue / Math.max(1, child.node.visits);
