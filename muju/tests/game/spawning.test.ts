...
describe('Spawning System', () => {
  ...
  describe('hasEnemyInRectangle', () => {
    it('returns false for empty rectangle', () => {
      ...
      expect(hasEnemyInRectangle(rectangle, board, 'white')).toBe(false);
    });
    ...
    it('returns true when enemy unit is in rectangle', () => {
      ...
      expect(hasEnemyInRectangle(rectangle, board, 'white')).toBe(true);
    });

    it('detects enemies from AI perspective', () => {
      let board = createEmptyBoard();
      const player = createUnit('p1', 'white', { x: 8, y: 8 });
      board = placeUnit(board, player);
      const rectangle = [{ x: 7, y: 7 }, { x: 8, y: 8 }, { x: 9, y: 9 }];

      expect(hasEnemyInRectangle(rectangle, board, 'black')).toBe(true);
    });
  });
  ...
  describe('isSpawnBlocked', () => {
    it('returns false when spawn zone has no enemies', () => {
      ...
      expect(isSpawnBlocked(anchor, 'white', board)).toBe(false);
    });
    ...
    it('returns true when enemy is in spawn rectangle', () => {
      ...
      expect(isSpawnBlocked(anchor, 'white', board)).toBe(true);
    });

    it('works for AI spawn zones', () => {
      let board = createEmptyBoard();
      const anchor = createUnit('a1', 'black', { x: 7, y: 7 });
      const player = createUnit('p1', 'white', { x: 8, y: 8 });
      board = placeUnit(board, anchor);
      board = placeUnit(board, player);

      expect(isSpawnBlocked(anchor, 'black', board)).toBe(true);
    });
  });
  ...
  describe('getSpawnZone', () => {
    ...
    it('excludes positions with friendly units', () => {
      let board = createEmptyBoard();
      const anchor = createUnit('p1', 'white', { x: 2, y: 2 });
      const friendly = createUnit('p2', 'white', { x: 1, y: 1 });
      board = placeUnit(board, anchor);
  ...
  describe('getValidAnchors', () => {
    it('returns all player units when none are blocked', () => {
      let board = createEmptyBoard();
      const unit1 = createUnit('p1', 'white', { x: 2, y: 2 });
      const unit2 = createUnit('p2', 'white', { x: 3, y: 1 });
      board = placeUnit(board, unit1);
      board = placeUnit(board, unit2);

      const anchors = getValidAnchors('white', board);
      expect(anchors.length).toBe(2);
    });

    it('excludes units whose spawn zones are blocked', () => {
      let board = createEmptyBoard();
      const unit1 = createUnit('p1', 'white', { x: 4, y: 4 }); // Blocked by enemy
      const unit2 = createUnit('p2', 'white', { x: 1, y: 0 }); // Not blocked
      const enemy = createUnit('e1', 'black', { x: 2, y: 2 }); // Blocks unit1
      board = placeUnit(board, unit1);
      board = placeUnit(board, unit2);
      board = placeUnit(board, enemy);
  ...
  describe('getAllSpawnPositions', () => {
    it('returns all empty positions from valid spawn zones', () => {
      ...
      const positions = getAllSpawnPositions('white', board);
    ...
    it('deduplicates overlapping spawn zones', () => {
      ...
      const unit1 = createUnit('p1', 'white', { x: 2, y: 2 });
      const unit2 = createUnit('p2', 'white', { x: 3, y: 3 });
      board = placeUnit(board, unit1);
      board = placeUnit(board, unit2);
  ...
  describe('isValidSpawnPosition', () => {
    it('returns true for empty position in valid spawn zone', () => {
      ...
      expect(isValidSpawnPosition({ x: 1, y: 1 }, 'white', board)).toBe(true);
    });
    ...
    it('returns false for position outside all spawn zones', () => {
      ...
      expect(isValidSpawnPosition({ x: 5, y: 5 }, 'white', board)).toBe(false);
    });
  ...
  describe('getLargestSpawnZone', () => {
    it('returns the largest available spawn zone', () => {
      ...
      const unit1 = createUnit('p1', 'white', { x: 1, y: 1 });
      // Unit at (3,3) has 16-cell spawn zone (4x4)
      const unit2 = createUnit('p2', 'white', { x: 3, y: 3 });
      board = placeUnit(board, unit1);
      board = placeUnit(board, unit2);

      const { anchor, zone } = getLargestSpawnZone('white', board);
