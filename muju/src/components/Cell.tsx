...
export function Cell({ cell, isValidMove, isValidAttack, isValidSpawn, isSelected, isInvalidSpawn, isPendingMove, movementRangeActions, isAttackFrontier, isPreview, showResources, moveCost, unitLabel, previewLabel, pendingLabel, onClick }: CellProps) {
  ...
  const label = `${coord}${home ? `, ${home} home corner` : ''}${unitLabel ? `, ${unitLabel}` : ''}${pendingLabel ? `, ${pendingLabel}` : ''}, ${crystalDescription}${previewLabel ? `, preview: ${previewLabel}` : ''}${reach ? `, move costs ${moveCost} actions` : ''}${isAttackFrontier ? ', enemy attack frontier: up to 3 move actions and 1 attack' : ''}${isValidAttack ? ', attack target' : ''}${isValidSpawn ? ', available for placement' : ''}`;
