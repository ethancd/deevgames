// Reproduce from muju/: node --import tsx lab/ai/results/t2c-gate1-pilot-2026-09-19/audit.mjs
...
const out = 'lab/ai/results/t2c-gate1-pilot-2026-09-19';
...
const normalize = s => ({
  ...
  res: Object.fromEntries(['white', 'black'].map(p => [p, { r: s.players[p].resources, g: s.players[p].resourcesGained, s: s.players[p].resourcesGained - s.players[p].resources }])),
  turn: s.turn.turnNumber, phase: s.turn.phase, actionsRemaining: s.turn.actionsRemaining,
});
await withHeavySlot('gate1-evidence-replay-audit', async () => {
  ...
  assert.equal(sha(readFileSync(`${out}/preregistration-A1.md`)), identity.preregistration.sha256);
  assert.equal(identity.preregistration.id, 'A1');
  // Normalize JSON number semantics: Elo can produce -0, which serializes as 0.
  const recomputed = JSON.parse(JSON.stringify(summarize(entries, 'pilot', manifest.identityHash, bands)));
  ...
  for (const e of entries) {
    ...
    let completedTurns = 0; const buys = { white: 0, black: 0 };
    for (const step of replay.steps.slice(1)) {
      ...
      assert.deepEqual(normalize(state), { units: step.units, pendingSummons: step.pendingSummons, cells: step.cells, res: step.res, turn: step.turn, phase: step.phase, actionsRemaining: step.actionsRemaining });
      actions++;
    }
    assert.equal(completedTurns, e.record.completedTurns);
    for (const p of ['white', 'black']) assert.equal(buys[p], e.record.players[p].unitsPlaced);
  ...
  const report = { checkedAt: new Date().toISOString(), sourceHashesVerified: Object.keys(identity.files).length,
    ...
    summaryRecomputed: true, purchasesAndCompletedTurnsVerified: true, adoptedA1HashVerified: true, identityHashVerified: true, workTurnsAudited: workTurns,
