...
/**
 * Passive income is the same rule under either turn, but Phasing is the only
 * ruleset since 2026-09-21, so every live state here says so; under it
 * `END_ACTION_PHASE` is the moment mining settles, inside the mover's own turn.
 */
describe('passive income',()=>{
 ...
 it('collects only for the mover at the final position and never consumes actions',()=>{
  const state=createInitialGameState(undefined,4,0,'phasing'),u=state.board.units.find(u=>u.owner==='white'&&u.definitionId==='water_1')!;
