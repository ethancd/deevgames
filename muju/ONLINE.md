...
the invitation to your opponent. New invitations use `<host>/join/abcdef`, with
six random lowercase letters. Codes survive restarts and claim the free seat once;
older invitation links still work. The game starts when they claim the other seat.
...
**Share watch link** in any online room provides `<host>/watch/abcdef`, with six
lowercase letters. Watch codes are persistent and separate from invitation codes;
existing rooms receive a watch code when opened. Older `&watch=1` links still work.
