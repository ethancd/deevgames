...
7. To rearrange the bag, select an item and tap an empty bag slot. Empty slots highlight with a move arrow. The item moves there and selection clears, without a gameplay request or time cost. Tapping another occupied slot still selects that item; it does not swap or overwrite anything. Keyboard Enter/Space works too.
