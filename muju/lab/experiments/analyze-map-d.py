...
import json, pathlib, random, statistics, collections, sys, gzip
...
baseline='S' if mode=='topology' else 'A'
files=sorted(out.glob(f'{mode}-*.jsonl')) or sorted(out.glob(f'{mode}-*.jsonl.gz'))
rows=[json.loads(l) for p in files for l in (gzip.open(p,'rt').read() if p.suffix=='.gz' else p.read_text()).splitlines()]
...
for c in sorted(set(r['cell'] for r in rows)):
 rs=[r for r in rows if r['cell']==c]; groups={m:[r for r in rs if r['map']==m] for m in [baseline,'D']}
 blocks=[]
 for i in sorted(set(r['i'] for r in rs)):
  z={m:[r for r in groups[m] if r['i']==i] for m in groups}
  if all(len(z[m])==2 for m in z):blocks.append(mean([score(r) for r in z['D']])-mean([score(r) for r in z[baseline]]))
 if not blocks:continue
 cells.append(dict(cell=c,a=rs[0]['a'],b=rs[0]['b'],**{baseline:stats(groups[baseline]),'D':stats(groups['D'])},pairedScoreDeltaPP=100*mean(blocks),ci95=boot(blocks)))
bybot={}
