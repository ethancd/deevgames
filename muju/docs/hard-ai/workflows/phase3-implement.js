...
6. If pass: commit ONLY this milestone's files: 'cd ${WT} && git add -- <each created/modified path, plus docs/hard-ai/design/DEVIATIONS.md if the implementer touched it> && git -c user.name=Claude -c user.email=noreply@anthropic.com commit -m "Hard AI ${m.id}: ${m.title}" -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PkrS9DRcQ7gWhR1LbovT5D"'. If git reports index.lock, wait 5 seconds and retry up to 10 times. Report the hash.
...
for (const m of MILESTONES) {
  promises[m.id] = (async () => {
    ...
    let impl = await agent(implPrompt(m, null), { label: `impl:${m.id}`, phase: phaseName, schema: IMPL_SCHEMA, model: m.model, effort: 'high' })
