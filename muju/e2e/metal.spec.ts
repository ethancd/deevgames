...
for (const width of [1280,390]) test(`Poṉ's stationary moves, stats and adjacent attack at ${width}px`, async ({page},info) => {
  ...
  await expect(yan).toHaveAttribute('aria-label',/white Poṉ/);
  await yan.click();
  ...
  await expect(page.getByTestId('cell-2-3')).not.toHaveAttribute('aria-label',/black Sjór/);
