...
for(const d of UNIT_DEFINITIONS)Object.assign(d,patch[d.id]??{});
...
try{for(let cell=0;cell<pairs.length;cell++){
 ...
 for(let i=0;i<n;i++)for(const swapped of [false,true]){
  ...
  const {record:r}=await playGame({bots:{white:bot(swapped?b:a),black:bot(swapped?a:b)},seed,engineHash:sourceHash,runId:`e8-${variant}`,experiment:variant,options:{legality:'strict',checkInvariants:true,maxTurns:variant==='engine'?30:120,recordReplay:false}});
