...
Next research step: run the UI-budget adapter against production and diverse opponents on a held-out seed block, inspect capped Balanced/AntiRush matchups, then measure a real mobile device before tuning the effort ladder. Deployment is a separate action: fetch current master, build the complete site from a compatible tree, and verify the immutable deployment plus all three games if publishing is later requested.
