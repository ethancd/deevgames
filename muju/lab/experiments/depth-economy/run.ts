...
for(let cell=0;cell<PAIRS.length;cell++){
 const [a,b]=PAIRS[cell];for(let i=0;i<n;i++)for(const swapped of [false,true]){
  ...
  const id=`${label==='scripted'?'':label+'-'}${economy}-${cell}-${i}-${Number(swapped)}`;
  const result=await playGame({bots:{white:policy(swapped?b:a),black:policy(swapped?a:b)},seed,engineHash:sourceHash,runId:id,initialState:initial,
   ...
   onAction(before,after,action,player){t.onAction(before,after,action,player);
    if(action.type==='PROMOTE_UNIT'&&before!==after){const u=before.board.units.find(u=>u.id===action.unitId)!;
     if(u.definitionId==='plant_2'){
      ...
      if(['scripted','sustain'].includes(label)&&cell<8&&player===aSeat&&!captured.has(key)){captured.add(key);append(`${label==='scripted'?'':label+'-'}origins-${economy}`,{id,cell,a,b,i,swapped,seed,aSeat,unitId:u.id,opportunity,state:before});}
     }
    }
   }});
