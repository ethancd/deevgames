Coordinator decision, 2026-09-19: approved as the pre-outcome implementation contract for an isolated M6 accounting bootstrap. This is a source-work authorization, not a passing evaluation or release claim. No engine outcomes informed these coefficients. Preserve the named 62-feature schema and catalogue principal accounting; zero coefficients remain explicit until separately preregistered. All canonical reference, parity, conservation, stage/bound and performance checks remain required. Any proposal deviation must be recorded and reviewed before outcomes. No corpus, tuning, val/sealed or release is authorized by this approval. M5 numeric floors/goldens remain to be frozen before measured bootstrap/data generation.
...
**Approved policy limitation (coordinator, 2026-09-19):** canonical `defaultUpkeepAction(state, false)` breaks equal-catalogue-cost ties by absolute board square. Rotation plus a seat swap can therefore select a different retained body when only one of two tied rent-paying units is affordable, producing different later mining and forecast values. For example, equal-cost Water II at C3 over remaining reserves and Shadow II at D4 on dry ground exchange tie order under rotation. Preserve this named policy in both implementations; do not replace its tie rule to force a symmetry assertion. Validate each transformed state independently against the canonical policy. Side-of-view sign antisymmetry for one fixed state still holds, but rotation/seat equivariance of this forecast or full evaluation is not claimed. Bootstrap book identity must consequently use the raw position key, not rotation canonicalization.
...
Implementation clarification from pre-outcome review: the movement window comes from the existing chronological forecast's first reached enemy Act before the owner's root batch resolves. Use that snapshot's surviving anchors, blockers, root-live mover identities, flags and AP; a root anchor released at outgoing upkeep is unavailable. New arrivals remain blockers but are excluded as movers from this bounded probe. Require the mover to be retainable at its ensuing bill: current bank plus exact finite mining income of every own live body, with only the mover relocated to its endpoint, must cover at least that mover's rent. Free movers always satisfy this condition. The owner may prioritize the mover and release other renters, so total army rent or the default keep choice would be an unjustified filter. Exclude necessarily released intruders from both risk and pressure. This arithmetic needs no new forecast/prover calls. It remains a conservative endpoint probe: captures, coordinated actions, alternate post-move keep effects and a full terminal counterproof are outside its scope, so pressure is a conditional opportunity rather than verified arrival denial.
...
Implementation clarification from pre-outcome review: compare current-horizon attack tables on the forecast's actual first incoming owner Act after the root batch resolves and healing completes. A terminal stop before that Act gives no future opportunity. The live-only comparator suppresses only newly arrived friendly attacker slots identified by root birth identity; it preserves their occupancy. Count only surviving root-live enemy identities, so reused dead slots cannot become ghost victims. Slot29 reports these named arrival-dependent victims; slot28 excludes those same root identities from its legacy static optimistic hanging set. Slot28's residual is still a different-horizon diagnostic, not a newly claimed chronological kill proof. All three corresponding weights remain zero.
