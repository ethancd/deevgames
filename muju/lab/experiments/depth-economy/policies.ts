...
export function makeBot(name:string):Bot {
 if(name.startsWith('Sustain:')){
  ...
  return {...base,name,chooseAction(ctx){
   ...
   const target=/^(Plant|Metal)([1-3])$/.exec(name.slice(8));
   const leader=target?ctx.view.board.units.filter(u=>u.owner===ctx.view.player&&def(u.definitionId).element===target[1].toLowerCase()).sort((a,b)=>def(b.definitionId).tier-def(a.definitionId).tier)[0]:undefined;
   const promotionReserve=leader&&target&&def(leader.definitionId).tier<Number(target[2])?(getPromotionCost(leader)??0):0;
   const legal=ctx.legal.filter(a=>{
    if(a.type==='QUEUE_UNIT'){const d=def(a.definitionId);return ctx.view.me.resources-d.cost>=rent+d.tier-1+promotionReserve;}
    return true;
   });
