...
import { sprt, eloToScore, type SprtParams } from '../../lab/hard-ai/ladder/sprt';
...
/**
 * Independent reference implementation of DESIGN §7.7's pentanomial SPRT
 * formula, re-derived here (not imported from `sprt.ts`) so the pin below
 * actually checks the module's arithmetic rather than comparing it to
 * itself. `counts` gives the number of pairs at each of the five pair-score
 * values `[0, 0.5, 1, 1.5, 2]`, in that order.
 */
function referenceLLR(counts: [number, number, number, number, number], params: SprtParams): number {
  const values = [0, 0.25, 0.5, 0.75, 1]; // pairScore/2
  const n = counts.reduce((a, b) => a + b, 0);
  let sum = 0;
  for (let i = 0; i < 5; i++) sum += counts[i] * values[i];
  const mu = sum / n;
  let sq = 0;
  for (let i = 0; i < 5; i++) sq += counts[i] * (values[i] - mu) ** 2;
  const variance = Math.max(sq / n, 1e-9);
  const t0 = 1 / (1 + 10 ** (-params.elo0 / 400));
  const t1 = 1 / (1 + 10 ** (-params.elo1 / 400));
  return (n * (t1 - t0) * (2 * mu - t0 - t1)) / (2 * variance);
}
...
describe('lab/hard-ai/ladder/sprt', () => {
  const cases: Array<{ name: string; counts: [number, number, number, number, number]; params: SprtParams }> = [
    { name: 'uniform-ish, elo0=0/elo1=50', counts: [3, 5, 8, 5, 3], params: { elo0: 0, elo1: 50, alpha: 0.05, beta: 0.05 } },
    { name: 'skewed toward A, elo0=0/elo1=100', counts: [1, 2, 4, 10, 15], params: { elo0: 0, elo1: 100, alpha: 0.05, beta: 0.05 } },
    { name: 'skewed toward B, elo0=-50/elo1=0', counts: [15, 10, 4, 2, 1], params: { elo0: -50, elo1: 0, alpha: 0.1, beta: 0.05 } },
    { name: 'all-draws (variance-floor path)', counts: [0, 0, 24, 0, 0], params: { elo0: 0, elo1: 10, alpha: 0.05, beta: 0.05 } },
  ];

  for (const c of cases) {
    it(`pins the LLR for "${c.name}" to 1e-9 against an independently re-derived formula`, () => {
      const pairScores = expandCounts(c.counts);
      const result = sprt(pairScores, c.params);
      const expected = referenceLLR(c.counts, c.params);
      expect(result.llr).toBeCloseTo(expected, 9);
      expect(result.n).toBe(pairScores.length);
    });
  }

  it('is exactly zero (to 1e-9) when the pair-score mean sits at the (t0, t1) midpoint under symmetric elo bounds', () => {
    // score(-e) + score(e) = 1 for any e (logistic symmetry), so with
    // elo0 = -elo1 the midpoint (t0+t1)/2 is exactly 0.5 in real-number
    // arithmetic. A counts distribution symmetric about the middle bucket —
    // count(pairScore=0) === count(pairScore=2) AND count(pairScore=0.5) ===
    // count(pairScore=1.5), any count(pairScore=1) — has mean(pairScore/2)
    // === 0.5 exactly, so 2μ̂ − t0 − t1 ≈ 0 and LLR ≈ 0 regardless of N or elo1.
    const params: SprtParams = { elo0: -73, elo1: 73, alpha: 0.05, beta: 0.05 };
    const counts: [number, number, number, number, number] = [7, 5, 6, 5, 7]; // symmetric about index 2
    const pairScores = expandCounts(counts);
    const result = sprt(pairScores, params);
...
describe('lab/hard-ai/ladder/elo', () => {
  ...
  it('eloEstimate on an even split of wins/losses returns ~0 Elo with los ~0.5', () => {
    ...
    expect(est.n).toBe(80); // 40 pairs * 2 games
  ...
  it('eloEstimate on a near-total A-wins run returns a large, finite positive Elo and los near 1', () => {
    // 29 pure A sweeps plus one split pair keeps mu just under 1 so elo stays
    // finite (mu === 1 exactly hits the +Infinity boundary case instead).
