...
it.each([{oldMap: PRE_CENTRAL_MAP}, {oldMap: PRE_EXPANSION_MAP}])('retains an in-progress old-map room across restart and uses the new map only for new rooms',({oldMap})=>{
 ...
 try{
  ...
  saved.state=endTurn(createInitialGameState(oldMap));
  ...
  expect(resumed.state.board.initialResourceLayers).toEqual(oldMap);
  ...
  expect(next.state.board.initialResourceLayers).toEqual(oldMap);
  const fresh=store.create({name:'New player'});
  expect(fresh.room.state.board.initialResourceLayers).toEqual(UNEQUAL_ROUTES_MAP);
  expect(fresh.room.state.board.cells.flat().reduce((n,c)=>n+c.resourceLayers,0)).toBe(504);
