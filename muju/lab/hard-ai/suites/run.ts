/**
 * `npm run hard:suite -- --suites <names> --engine <name> [--work <units>]
 *   [--shards <n>] [--out <path>]` (DESIGN §7.5, MILESTONES.md M14).
 *
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--suites') args.suites = argv[++i].split(',').map(s => s.trim()).filter(Boolean);
 ...
 * penalty features. */
function evaluateFor(state: GameState, side: Side): number {
  const p = PAIR_REPLICA.pack(state, PAIR_SCRATCH_STATE);
  PAIR_EVAL.invalidate();
  return PAIR_EVAL.full(p, side, PAIR_SCRATCH, 0);
}
...
const PAIR_EVAL = new Evaluator(PAIR_REPLICA);

async function runShard(args: Args, items: readonly WorkItem[]): Promise<Partial> {
  const partial = emptyPartial();
  const patch = hardConfigFor(args.engine.startsWith('hard@') ? args.engine.slice('hard@'.length) : args.engine);
  for (const item of items) {
    ...
    if (item.pair !== undefined) {
      ...
      const evalViolating = evaluateFor(item.pair.violating, side);
      const evalCorrect = evaluateFor(item.pair.correct, side);
