...
OUT=Path(__file__).resolve().parent; ROOT=OUT.parents[2]; MUJU=ROOT/'muju'
DATA=MUJU/'lab/results/opening-census-2026-09-14'
A=json.loads((DATA/'analysis.json').read_text());C=json.loads((DATA/'census.json').read_text())
for name,expected in C['hashes'].items():
    assert hashlib.sha256((MUJU/name).read_bytes()).hexdigest()==expected, f'Rule changed: {name}'
readme='''MHT v2.8 opening census - 14 September 2026
...
fields=['id','coords','blackCoords','minActions','shortestSequences','bank','take','reserves','next','second','spawnCount','spawnReserve','order','orderName','genus','species','demands','zones','spawnBand','economy','space','captureLiability','witnessText']
buf=io.StringIO();wr=csv.writer(buf);wr.writerow(fields+['blackWitness'])
for f in A['features']:
    values=[json.dumps(f[k],separators=(',',':')) if isinstance(f[k],list) else f[k] for k in fields]
    witness='; '.join(['Hi','Sjor','Muju'][u]+' '+chr(65+(99-n)%10)+str((99-n)//10+1) for u,n in f['witness']) or 'End turn'
    wr.writerow(values+[witness])
members={
 'README.txt':readme.encode(), 'all-W1-states.csv':buf.getvalue().encode(),
 'all-B1-states.csv':gzip.decompress((DATA/'all-B1-states.csv.gz').read_bytes())}
for name in ['census.json','analysis.json','joint.bin','verification.json','continuations.json']:
    members[name]=(DATA/name).read_bytes()
source_files=[]
for tree in ['src','server','lab/experiments/opening-census-2026-09-14']:
    source_files += [p for p in (MUJU/tree).rglob('*') if p.is_file()]
source_files += [MUJU/p for p in ['SPEC.md','package.json','package-lock.json','tsconfig.json','server/notation.ts']]
for f in source_files:
    if f.exists():members['source/muju/'+str(f.relative_to(MUJU))]=f.read_bytes()
members['builders/build-workbook.mjs']=(OUT/'build-workbook.mjs').read_bytes()
members['builders/build-report.py']=(OUT/'build-report.py').read_bytes()
manifest={name:{'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest()} for name,data in members.items()}
members['manifest.json']=json.dumps(manifest,indent=2).encode()
archive=OUT/'MHT-opening-raw-data.zip'
...
rows=list(csv.DictReader(io.StringIO(members['all-B1-states.csv'].decode())))
assert len(rows)==635203
assert len({r['State_ID'] for r in rows})==635203
from collections import Counter
assert dict(Counter(r['Grade'] for r in rows))==A['gradeCounts']
for r in rows:
    assert len({r[k] for k in ['W_Hi','W_Sjor','W_Muju','B_Hi','B_Sjor','B_Muju']})==6
with zipfile.ZipFile(archive) as z:
    assert z.testzip() is None
    for name,d in manifest.items():assert hashlib.sha256(z.read(name)).hexdigest()==d['sha256']
(OUT/'qa/package-validation.json').write_text(json.dumps({'sourceHashesUnchanged':True,'csvRows':len(rows),'uniqueStates':len(rows),'archiveMembers':len(members),'verifiedAllManifestHashes':True,'gradeCounts':A['gradeCounts']},indent=2))
print(f'Packaged {len(members)} members, {len(rows):,} unique states, {archive.stat().st_size:,} compressed bytes')
