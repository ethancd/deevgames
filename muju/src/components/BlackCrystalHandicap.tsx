...
export function BlackCrystalHandicap({ value, onChange, phasing = false }: { value: number; onChange: (value: number) => void; phasing?: boolean }) {
  const id = useId();
  return <div className="space-y-2">
    ...
    <p id={`${id}-hint`} className="text-sm text-gray-400">
      {phasing ? `Black starts with ${value} crystals. Both players begin with actions; spend after mining and upkeep. White still moves first.` : value > 0
