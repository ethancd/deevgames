...
export function createMcpServer(backend: RoomBackend, publicUrl: string, scope?: MatchScope) {
  ...
  server.registerResource('muju-micro-rules', 'muju://rules/micro', { mimeType: 'application/json', description: 'MICRO MUJU rules (6×6, three pieces, two actions)' },
    async uri => ({ contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(rulesFor('micro')) }] }));
  ...
  if (matchToolAllowed('muju_rules', scope)) server.registerTool('muju_rules', { description: 'Read the rules, unit stats, coordinates, and workflow before playing. Actions first, end-of-action mining/upkeep, public delayed summons and END_PLACE_PHASE handoff. Pass variant "micro" for MICRO MUJU rooms (observation variant "micro").',
    inputSchema: { ruleset: z.literal('phasing').optional().describe('Deprecated: Phasing is the only ruleset; omit.'),
      variant: z.literal('micro').optional().describe('"micro" for MICRO MUJU (6×6, three pieces, two actions). Omit for Muju Hono Irumbu.') }, annotations: readOnly },
